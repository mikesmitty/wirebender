...
func main() {
	...
	speedStraight := flag.Int("speed-straight", 0, "Servo speed for feed-only moves (0 = use -speed)")
	speedBend := flag.Int("speed-bend", 0, "Servo speed for bend moves (0 = use -speed)")
	verbose := flag.Bool("v", false, "Verbose output")
	strict := flag.Bool("strict", false, "Exit with error when segments are too short for bend radius")
	...
	gcode, err := generateGCode(points, *feedScale, effSpeedStraight, effSpeedBend, *springbackM, *springbackO, totalRadius, *strict, *verbose)
	if err != nil {
...
func generateGCode(points []mgl64.Vec3, feedScale float64, speedStraight int, speedBend int, springbackM float64, springbackO float64, totalRadius float64, strict bool, verbose bool) (string, error) {
	var out string
